    ...
    try {
        if ($action === 'save_work') {
            ...
            if (empty($_POST['work_id'])) {
                $stmt = $pdo->prepare("INSERT INTO subcontractor_works (subcontractor_id, project_id, work_reference, responsible, total_exc_vat, total_inc_vat, certified_total_inc_vat, invoiced_value, invoice_ref, payment_date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([ $sub_id, $project_id, trim($_POST['work_reference']), trim($_POST['responsible']), $_POST['total_exc_vat'] ?: 0, $_POST['total_inc_vat'] ?: 0, $_POST['certified_total_inc_vat'] ?: 0, $_POST['invoiced_value'] ?: 0, trim($_POST['invoice_ref']), !empty($_POST['payment_date']) ? $_POST['payment_date'] : null, trim($_POST['notes']) ]);
                $message = "Work summary added successfully!";
            } else {
                $stmt = $pdo->prepare("UPDATE subcontractor_works SET project_id=?, work_reference=?, responsible=?, total_exc_vat=?, total_inc_vat=?, certified_total_inc_vat=?, invoiced_value=?, invoice_ref=?, payment_date=?, notes=? WHERE id=? AND subcontractor_id=?");
                $stmt->execute([ $project_id, trim($_POST['work_reference']), trim($_POST['responsible']), $_POST['total_exc_vat'] ?: 0, $_POST['total_inc_vat'] ?: 0, $_POST['certified_total_inc_vat'] ?: 0, $_POST['invoiced_value'] ?: 0, trim($_POST['invoice_ref']), !empty($_POST['payment_date']) ? $_POST['payment_date'] : null, trim($_POST['notes']), $_POST['work_id'], $sub_id ]);
                $message = "Work summary updated successfully!";
            }
        } 
        elseif ($action === 'delete_work') {
            $pdo->prepare("DELETE FROM subcontractor_works WHERE id=?")->execute([$_POST['work_id']]);
            $message = "Work record deleted.";
        }
        elseif ($action === 'save_transaction') {
            $sub_id = $_POST['subcontractor_id'];
            if (empty($_POST['transaction_id'])) {
                $stmt = $pdo->prepare("INSERT INTO subcontractor_transactions (subcontractor_id, transaction_date, transaction_type, amount, reference, notes, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([ $sub_id, $_POST['transaction_date'], $_POST['transaction_type'], $_POST['amount'] ?: 0, trim($_POST['reference']), trim($_POST['notes']), getCurrentUserId() ]);
                $message = "Transaction logged successfully!";
            } else {
                $stmt = $pdo->prepare("UPDATE subcontractor_transactions SET transaction_date=?, transaction_type=?, amount=?, reference=?, notes=? WHERE id=? AND subcontractor_id=?");
                $stmt->execute([ $_POST['transaction_date'], $_POST['transaction_type'], $_POST['amount'] ?: 0, trim($_POST['reference']), trim($_POST['notes']), $_POST['transaction_id'], $sub_id ]);
                $message = "Transaction updated successfully!";
            }
        }
        elseif ($action === 'delete_transaction') {
            $pdo->prepare("DELETE FROM subcontractor_transactions WHERE id=?")->execute([$_POST['transaction_id']]);
...
<div class="main-container">
    ...
    <?php if (!$sub_id): ?>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <div>
                ...
                <p style="color: var(--text-secondary); margin-top: 0.25rem;">Overview of total certified works vs total payments.</p>
            </div>
        </div>

        <?php
        // Complex query to sum up works and transactions per subcontractor
        $ledgerStmt = $pdo->query("
            SELECT 
                s.id, s.name, s.contact_person,
                COALESCE((SELECT SUM(certified_total_inc_vat) FROM subcontractor_works w WHERE w.subcontractor_id = s.id), 0) as total_certified,
                COALESCE((SELECT SUM(amount) FROM subcontractor_transactions t WHERE t.subcontractor_id = s.id AND t.transaction_type = 'Payment'), 0) as total_paid
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <a href="?sub_id=<?= $l['id'] ?>" class="btn btn-sm btn-secondary">View Account</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
    ...
    <?php else: ?>
        ...
        // Fetch Works
        $wStmt = $pdo->prepare("SELECT w.*, p.name as project_name FROM subcontractor_works w LEFT JOIN projects p ON w.project_id = p.id WHERE w.subcontractor_id = ? ORDER BY w.id DESC");
        $wStmt->execute([$sub_id]);
        $works = $wStmt->fetchAll();

        // Fetch Transactions
        $tStmt = $pdo->prepare("SELECT * FROM subcontractor_transactions WHERE subcontractor_id = ? ORDER BY transaction_date DESC, id DESC");
        $tStmt->execute([$sub_id]);
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <div>
                <a href="subcontractor_accounts.php" style="color: var(--text-muted); text-decoration: none; font-size: 0.9rem;">&larr; Back to Ledger</a>
        ...
        <div class="table-container" style="margin-bottom: 3rem;">
            <table class="data-table">
                ...
                <tbody>
                    <?php if (empty($works)): ?>
                        <tr><td colspan="<?= $canManage ? '8' : '7' ?>" style="text-align: center;">No works certified yet.</td></tr>
                    <?php else: ?>
                        <?php foreach($works as $w): ?>
                        <tr>
                            <td>
                                <div style="font-weight: 600;"><?= htmlspecialchars($w['work_reference']) ?></div>
                                <div style="font-size: 0.8rem; color: var(--text-muted);"><?= htmlspecialchars($w['project_name'] ?? 'No Project') ?></div>
                            </td>
            ...
            <table class="data-table">
                ...
                <tbody>
                    <?php if (empty($transactions)): ?>
                        <tr><td colspan="<?= $canManage ? '6' : '5' ?>" style="text-align: center;">No transactions logged yet.</td></tr>
                    <?php else: ?>
