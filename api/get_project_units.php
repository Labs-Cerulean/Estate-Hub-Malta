...
try {
    ...
    if (empty($units)) {
        ...
        foreach ($units as $u) {
            ...
            // Increased margin-bottom to physically separate the cards more
            ...
            if ($is_manager) {
                $html .= "<button class='btn btn-link p-0 mt-3 d-block w-100 text-end' style='font-size: 0.8rem; text-decoration: none; color: {$accentColor};' onclick='togglePriceEdit({$u['id']})'><i class='fas fa-pen'></i> Modify Pricing</button>";
            }
            $html .= "</div>";

            // --- MANAGER EDIT PRICE FORM (Stacked for breathing room) ---
            if ($is_manager) {
                $html .= "
                        <div id='price_edit_{$u['id']}' class='mt-3 p-3' style='display:none; background: rgba(14, 165, 233, 0.05); border-radius: 10px; border: 1px solid rgba(14, 165, 233, 0.2);'>
                            ...
                            <div class='d-flex flex-column gap-2'>
                                <button class='btn btn-info w-100 py-2 text-dark fw-bold' style='font-size: 0.9rem; border-radius: 8px;' onclick='savePrice({$u['id']})'><i class='fas fa-save me-1'></i> Save</button>
                                <button class='btn btn-outline-secondary w-100 py-2' style='font-size: 0.9rem; border-radius: 8px;' onclick='togglePriceEdit({$u['id']})'><i class='fas fa-times me-1'></i> Cancel</button>
                            </div>
                        </div>";
            }

            // --- MODERN ACTION BUTTONS (Forced Vertical Stack) ---
            $planBtn = '';
            $floorLvl = trim($u['floor_level']);
            if (isset($plans[$floorLvl])) {
                $safeUrl = htmlspecialchars($plans[$floorLvl], ENT_QUOTES, 'UTF-8');
                $planBtn = "<button class='btn w-100' style='background: rgba(14, 165, 233, 0.1); color: #0ea5e9; border: 1px solid rgba(14, 165, 233, 0.3); border-radius: 8px; font-size: 0.9rem; font-weight: 600; padding: 12px 0;' onclick='openPlanModal(\"{$safeUrl}\")'><i class='fas fa-map me-1'></i> View Plan</button>";
            }

            // Using flex-column to stack everything cleanly
            $html .= "<div class='mt-4 d-flex flex-column gap-3'>"; 
            
            if ($is_manager) {
                // Manager Override Dropdown
                $statuses = ['Available', 'On Hold', 'Reserved', 'Sold - POS', 'Sold - Contract', 'Resale', 'BOM'];
                $html .= "<select class='form-select bg-dark text-light border-secondary w-100' style='font-size: 0.95rem; border-radius: 8px; padding: 12px;' onchange='managerUpdateStatus({$u['id']}, this.value, this)'>";
                ...
                // Agent Buttons
                if ($status === 'Available') {
                    $html .= "<button class='btn w-100' style='background: rgba(245, 158, 11, 0.1); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.3); border-radius: 8px; font-size: 0.9rem; font-weight: 600; padding: 12px 0;' onclick='holdProperty({$u['id']})'><i class='fas fa-hand-paper me-1'></i> Put on Hold</button>";
                } elseif ($status === 'On Hold' && $u['held_by_agent_id'] == $_SESSION['user_id']) {
                     $html .= "<button class='btn w-100' style='background: rgba(16, 185, 129, 0.1); color: #10b981; border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 8px; font-size: 0.9rem; font-weight: 600; padding: 12px 0;' onclick='requestReserve({$u['id']})'><i class='fas fa-check-circle me-1'></i> Reserve Unit</button>";
                }
                $html .= $planBtn;
            }
