        ...
        <div id="view-punch-out" class="view">
            ...
            <div style="background: #fff; padding: 20px; border-radius: 16px; margin-bottom: 30px; border: 1px solid #e2e8f0;">
                ...
                <div id="shift-confirm-box" style="display:none; background:#f0fdf4; padding:18px; border-radius:12px; margin-bottom:18px; border:1px solid #86efac;">
                    <h4 style="margin:0 0 6px; color:#166534; font-size:1rem;"><i class="fas fa-clipboard-check"></i> Confirm shifts</h4>
                    <p style="margin:0 0 14px; color:#4b5563; font-size:0.85rem;">Review the mode and add-on quantities for each shift. Adjust if needed before the client signs off.</p>
...
function addConfigRow(data = {type: 'mode', name: '', price: 0, nom_code: ''}) {
    ...
    function startPunchOut(id, pricingType) { 
        ...
        if (job && Number(job.has_configurations) === 1 && job.configurations) {
            ...
            fetch(`api/plant_actions.php?action=get_job_sessions&booking_id=${encodeURIComponent(id)}`)
                ...
                .then(data => {
                    if (!data || data.error) throw new Error(data?.error || 'Could not load shifts');
    ...
    function escapePunchHtml(value) {
        return String(value ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
    }

    function escapePunchAttr(value) {
        return escapePunchHtml(value).replace(/'/g, '&#39;');
    }
    ...
    function renderShiftConfirmationUI(data) {
        ...
        shiftList.innerHTML = data.shifts.map((shift, idx) => {
            ...
            const modeHtml = hasModes
                ? `<select class="shift-mode-select input-heavy" data-shift-idx="${idx}" style="margin-bottom:0;">${modes.map(m => `<option value="${escapePunchAttr(m.name)}" ${shift.mode_name === m.name ? 'selected' : ''}>${escapePunchHtml(m.name)}</option>`).join('')}</select>`
                : `<input type="hidden" class="shift-mode-select" data-shift-idx="${idx}" value="${escapePunchAttr(shift.mode_name || 'Standard Operation')}"><span style="font-weight:700;">${escapePunchHtml(shift.mode_name || 'Standard Operation')}</span>`;

            const addonHtml = addons.length > 0
                ? addons.map(a => `
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-top:8px;">
                        <span>${escapePunchHtml(a.name)}</span>
                        <input type="number" class="shift-addon-input" data-shift-idx="${idx}" data-addon-name="${escapePunchAttr(a.name)}" min="0" value="${addonMap[a.name] || 0}" style="width:70px; padding:6px; border:1px solid #cbd5e1; border-radius:6px; text-align:center; font-weight:bold;">
                    </div>
                `).join('')
                : '';
            ...
            return `
                <div class="shift-confirm-card" data-shift-idx="${idx}" data-session-id="${shift.id ?? ''}" style="background:#fff; border:1px solid #d1d5db; border-radius:12px; padding:14px; margin-bottom:10px;">
                    ...
                    ${modeHtml}
                    ${addonHtml ? `<div style="margin-top:12px; border-top:1px dashed #e5e7eb; padding-top:10px;"><div style="font-size:0.72rem; font-weight:700; text-transform:uppercase; color:#64748b; margin-bottom:4px;">Add-ons (qty)</div>${addonHtml}</div>` : ''}
                </div>
            `;
        }).join('');
    }
    ...
    function collectShiftConfirmations() {
        const cards = document.querySelectorAll('.shift-confirm-card');
        const confirmations = [];
        cards.forEach(card => {
            ...
            card.querySelectorAll('.shift-addon-input').forEach(input => {
                ...
                if (qty > 0) {
                    addons.push({ name: input.getAttribute('data-addon-name'), qty });
                }
            });
