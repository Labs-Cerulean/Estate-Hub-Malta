...
// ==========================================
function calculateCapitalKPIs($fin, $variations, $claims) {
...
if ($projectId) {
    ...
    if (!$project || $project['type'] !== '3rd-party') {
        header('Location: capital_projects.php'); exit;
    }
    ...
    $kpis = calculateCapitalKPIs($fin, $variations, $claims);
...
// ------------------------------------------
    ...
    // Fetch all financials at once to avoid N+1 query loops
...
.matrix-table tbody td:first-child { position: sticky; left: 0; background: #1e1e2d; z-index: 5; border-right: 2px solid var(--border-glass); font-weight: 700; }
...
<div class="main-container" style="max-width: <?= $projectId ? '1200px' : '100%' ?>; padding: 1.5rem;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1.5rem;">
        <div>
            ...
            <p style="color: var(--text-secondary); margin-top: 0.25rem;">
                <?= $projectId ? 'Manage base contract, track variations, and log interim payment certificates.' : 'Global financial overview of all active 3rd-Party Capital Projects.' ?>
            </p>
        </div>
        <?php if ($projectId): ?>
            ...
            <table class="matrix-table">
                <thead>
                    <tr>
                        ...
                        <th class="currency" style="border-left: 2px solid var(--border-glass);">Order Value (€)</th>
                        <th class="currency">Appr. Variations (€)</th>
                        <th class="currency">Total Contract (€)</th>
                        <th class="currency" style="color: #f59e0b;">Pending Variations (€)</th>
                        <th class="currency" style="border-left: 2px solid var(--border-glass);">Claimed Total (€)</th>
                        <th class="currency">Claimed Approved (€)</th>
                        ...
                        <?php foreach($matrixData as $row): $f = $row['fin']; $k = $row['kpis']; ?>
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <div style="font-size: 0.7rem; color: var(--text-muted); margin-bottom: 2px;"><?= $k['time_elapsed'] ?>%</div>
                                    <div style="height: 4px; background: rgba(255,255,255,0.1); border-radius: 2px; overflow: hidden; width: 60px; margin: 0 auto;">
                                        <div style="height: 100%; width: <?= $k['time_elapsed'] ?>%; background: <?= $k['time_elapsed'] > 90 ? '#ef4444' : 'var(--primary-color)' ?>;"></div>
                                    </div>
                                </td>
                                ...
                                <td class="currency" style="font-weight: 700; color: #10B981;"><?= number_format($k['total_value'], 2) ?></td>
                                ...
                                <td style="text-align: center; font-weight: 600;"><?= $k['pct_approved'] ?>%</td>
    ...
    <?php else: ?>
        ...
        <div class="stats-grid" style="margin-bottom: 2rem;">
            ...
            <div class="stat-card"><div class="stat-number" style="font-size: 1.5rem;"><?= $kpis['days_left'] ?></div><div class="stat-label">Days to Target (<?= $kpis['time_elapsed'] ?>%)</div></div>
        </div>

        <details class="custom-accordion" open>
            <summary>📄 Base Contract & Timeframes</summary>
            <div class="accordion-content">
                <form method="POST">
                    ...
                    <div class="form-grid" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem;">
                        ...
                        <div class="form-group"><label>Less Fines / Deductions (€)</label><input type="number" step="0.01" name="less_fines" value="<?= $fin['less_fines'] ?? '0.00' ?>" <?= $canEdit ? '' : 'disabled' ?>></div>
        ...
        <details class="custom-accordion" open>
            <summary>🔄 Variations Log</summary>
            <div class="accordion-content">
                <?php if ($canEdit): ?>
                                ...
                                <?php foreach ($variations as $v): ?>
                                    <tr>
                                        ...
                                        <td style="text-align: center;">
                                            <?php 
                                            $bClass = 'badge-gray';
                                            if ($v['status'] === 'Approved') $bClass = 'badge-green';
                                            if ($v['status'] === 'Pending') $bClass = 'badge-yellow';
                                            ?>
                                            <span class="badge <?= $bClass ?>"><?= $v['status'] ?></span>
                                        </td>
                                        <?php if ($canEdit): ?>
        ...
        <details class="custom-accordion" open>
            <summary>🧾 IPC Claims Log</summary>
            <div class="accordion-content">
                <?php if ($canEdit): ?>
                                ...
                                <?php foreach ($claims as $c): ?>
                                    <tr>
                                        ...
                                        <td style="text-align: center;">
                                            <?php 
                                            $bClass = 'badge-gray';
                                            if ($c['status'] === 'Paid') $bClass = 'badge-green';
                                            if ($c['status'] === 'Pending') $bClass = 'badge-yellow';
                                            ?>
        ...
        <div id="varModal" class="modal">
            <div class="modal-content">
                ...
                <form method="POST">
                    ...
                    <div class="form-group"><label>Amount (€)</label><input type="number" step="0.01" name="amount" id="varAmount" required></div>
        ...
        <div id="claimModal" class="modal">
            <div class="modal-content">
                ...
                <form method="POST">
                    ...
                    <div class="form-group"><label>Amount Claimed (€)</label><input type="number" step="0.01" name="amount_claimed" id="claimAmt" required></div>
                    <div class="form-group"><label>Amount Approved (€)</label><input type="number" step="0.01" name="amount_approved" id="claimApp" value="0.00"></div>
